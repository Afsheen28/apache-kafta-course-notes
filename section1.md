 ...
 * Message Key: Used for identifying messages, can be of various data types. Typically a String.
 * Message Value: Contains the event data. Typically in JSON.
...
* Message Structure: A Kafka message is typically viewed as a key-value pair, with both key and value being able to accommodate various data types, including null. For instance, a message key could be an alphanumeric string or have a custom format.
* Timestamp: Each message has a timestamp that indicates when the event occurred. This timestamp can either be generated by the system or manually set. Knowing the time an event happens is crucial for many use cases, such as auditing or event ordering.
* Headers: Kafka messages can also contain headers, which are optional key-value pairs used to carry additional metadata. For example, an authorization header could be necessary for a destination microservice to access protected resources. While headers can add valuable information, they are not always utilized.
* Main Message Components: The most frequently used elements of a Kafka message are the key, value, timestamp, and potentially headers. Understanding these components is crucial as they play vital roles in how messages are processed and consumed within the Kafka ecosystem.
* Partitioning: Events are stored in different partitions based on the presence of a message key. When no message key is provided, Kafka distributes events across available partitions for load balancing, which can lead to issues with event order during consumption.
* When the order of events is crucial—like during updates to a user's first name—using a message key is essential. By consistently using a message key, such as a user ID, Kafka ensures that all related events are stored in the same partition, preserving their order.
